...
const getProviderFactory = () => {
  return async (): Promise<RpcProvider> => {
    class FormatterAnvil extends Formatter {
    ...
    class Provider extends StaticJsonRpcProvider {
      ...
      static getFormatter(): Formatter {
        if (this._formatter == null) {
          this._formatter = new FormatterAnvil();
        }
        return this._formatter;
      }

      clone() {
        return new Provider(ANVIL_URL);
      }
    }
...
@Module({})
export class AnvilProviderModule {
  static forRoot(): DynamicModule {
    return {
      module: AnvilProviderModule,
      global: true,
      providers: [
        ProviderService,
        {
