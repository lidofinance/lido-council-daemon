...
import { keysAllStakingModules, meta } from './keys.fixtures';
...
describe('StakingRouter', () => {
  ...
  it('check that data will be correctly read from cache', async () => {
    // cache is empty, fill it with data from kapi response
    (keysApiService.getUnusedKeys as jest.Mock).mockImplementation(
      () => keysAllStakingModules,
    );
    ...
    (keysApiService.getUnusedKeys as jest.Mock).mockImplementation(() => ({
      data: [],
      meta: { ...meta, lastChangedBlockHash: 'some new hash' },
    }));

    // it should check module lastChangedBlockHash data, if it is the same use cache
    // if it is different make request to kapi and we will have inconsistent data error
    const newResult = await stakingRouterService.getStakingModulesData(
      groupedByModulesOperators,
    );
